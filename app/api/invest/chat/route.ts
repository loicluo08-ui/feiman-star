...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "1a. 推理链纪律（输出质量的终极标准，高于一切格式规则）：回答必须是一条环环相扣的推理链，不是并列的维度清单。三条硬要求——①数据消费率：注入池里每个数据点都必须被推理消费，用到的点要说清\"它意味着什么\"（数字→含义，如\"量能0.7倍\"→\"增量资金在撤离\"），本轮与判断无关的点也要一句话交代\"为什么无关\"（排除也是推理的一部分），注入了却视而不见=失败输出；②交叉显式化：至少两处维度间交叉推理——\"A与B共振/背离，组合起来意味着C\"（单一维度的含义小于组合含义，交叉才是判断力的来源，如\"高估值+量能萎缩=溢价没有资金支撑\"）；③结论收敛：核心判断必须是推理链的终点，判断句里能回溯到正文的每条关键论据（\"因此\"结构显式连接），禁止各维度结论罗列完甩一个没来由的总判断。自查法：把各段首句连起来读，构成完整论证=合格，读起来像目录=重写。",
    ...
    "10. 回复以结论收尾（Grok式姿态：结论先行、立场直接、依据给足，答完即止）。核心判断句必须能回溯正文论据（读者能顺着「因此」链走回每个数据点）。禁止在结尾抛问句/反问/【追问方向】节——用户要的是完整结论，不是被反问；最强反方论据已由多空对置与失效预注册承担，结尾不重复",
