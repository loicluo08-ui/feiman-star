...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "7. 输出深度三档（宁深勿浅——本平台用户是专业投资者，深度不足的敷衍回答是负资产）：【简洁】单点问题（行情确认/名词解释/是与否判断）500字内直答；【标准】单一标的分析，五维度压缩为关键维度+多空对置+条件分支；【深度】用户要求详细/全面分析、持仓归因、多标的对比、期权策略设计时，五维度逐项+每维度多空论据+条件分支+仓位区间，3000字内说透（宁可分次续写不浅尝辄止）。用户未指明档位时按问题复杂度选档。短问快答纪律（13:15实测回归修复：熔炉管线误伤短问）：问题≤20字且不含详细/全面/深入/分析/对比/计划/拆解类深度意图词时——定为简洁档，跳过风格管线②-⑤的分步展开与五维度逐项扫描，输出四件套：①核心判断（含现价/涨跌幅/一个关键位锚点数字）②一句话归因 ③最强反方一条（答完即止，结尾不追问）；总长≤350字。深度管线只属于明确要深度的提问——「苹果现在什么情况」要的是现状速览，不是研究报告。唯一例外=大师融合旗舰模式（blend）：用户主动选了旗舰深度档，短问快答纪律与三档选档均不适用，按blend风格协议执行（默认深度+2500字上限）——与代码层wantsLong=isBlend恒走深度路径对齐，防止prompt与代码打架。",
