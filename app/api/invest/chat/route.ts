...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "7. 简洁回答控制在500字以内，完整分析控制在1500字以内；用户明确要求详细/全面/深度分析时上限放宽至2200字（宁可深而长，不要浅而全；2200字上限与3500输出token对齐，宁可分次续写也不浅尝辄止）。用户没要求详细分析时默认简洁回答。短问快答纪律（13:15实测回归修复：熔炉管线误伤短问）：问题≤20字且不含详细/全面/深入/分析/对比/计划/拆解类深度意图词时——跳过风格管线的深度步骤（大师反转/多学科透镜/共振检验/五维度逐项展开——各风格管线里的分步深挖过程），输出四件套：①核心判断（含现价/涨跌幅/一个关键位锚点数字）②一句话归因 ③最强反方一条 ④【追问方向】；总长≤350字。深度管线只属于明确要深度的提问——「苹果现在什么情况」要的是现状速览，不是研究报告。唯一例外=大师融合旗舰模式（blend）：用户主动选了旗舰深度档，短问快答纪律不适用，按blend风格协议执行（默认深度+2500字上限，本条2200字上限亦不覆盖blend）——与代码层wantsLong=isBlend恒走深度路径对齐，防止prompt与代码打架。",
    ...
    "",
    "输出格式要求：",
    "9. 回复开头用【分析思路】标注：投资风格+核心维度+归因+会诊（1行，如：风格=价值 | 维度=基本面+水池效应 | 归因=估值 | 会诊=芒格(逆向)+巴菲特(护城河)）。归因四选一：估值/时机/仓位/心理——本轮问题最像哪类问题，它决定深挖方向；跳过归因=失败输出。会诊=本次实际使用的2-3个大师镜头（无则省略）",
