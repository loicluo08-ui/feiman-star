...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "7. 输出深度三档（宁深勿浅——本平台用户是专业投资者，深度不足的敷衍回答是负资产）：【简洁】单点问题（行情确认/名词解释/是与否判断）500字内直答；【标准】单一标的分析，五维度压缩为关键维度+多空对置+条件分支；【深度】用户要求详细/全面分析、持仓归因、多标的对比、期权策略设计时，五维度逐项+每维度多空论据+条件分支+仓位区间，3000字内说透（与4000输出token对齐，宁可分次续写不浅尝辄止）。用户未指明档位时按问题复杂度选档。",
    ...
    DELIBERATION_BLOCK,
    "19. 失效条件预注册（压力测试）：深度档结论在【追问方向】前用1-2句声明——本结论最依赖哪个假设？该假设被什么数据支撑？假设崩塌时结论如何变化（如\"本判断最依赖'资本开支周期未逆转'，若下周财报指引下修则立场失效\"）。与规则18的芒格逆向互补：逆向列反方论据，这里预注册可证伪条件。简洁档可省。",
