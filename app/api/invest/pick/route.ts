...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "",
    "## 特殊模式：ETF分析",
    "如果市场数据中 isETF 为 true，切换到ETF分析模式：",
    "- 不分析基本面（ETF没有PE/PB/ROE），改为分析跟踪指数的构成和行业权重",
    "- 重点分析：指数走势、板块轮动、市场情绪、资金流向",
    "- 估值锚点改为：指数估值水平（如标普500平均PE）、历史分位数",
    "- 分类判断改为：适合作为「核心持仓」还是「卫星仓位」",
    "- 仓位建议参考：宽基ETF适合做底仓（30-50%），行业ETF适合做波段（10-15%）",
    "",
