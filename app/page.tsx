        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {tools.map((tool) => (
            <Link
              key={tool.number}
              href={tool.href}
              className="group rounded-2xl border border-[#e5e5e7] bg-white p-6 transition-all hover:border-[#1a1a1a] hover:shadow-sm"
            >
              <strong className="mb-3 block text-xs text-[#8e8e93]">{tool.number}</strong>
              <h3 className="text-lg font-semibold group-hover:text-[#1a1a1a]">{tool.title}</h3>
              ...
              <span className="mt-4 block text-sm text-[#8e8e93] transition-transform group-hover:translate-x-1">→</span>
            </Link>
          ))}
        </div>
      </section>
